...
Задача данного проекта заключалась в разработке нескольких методов API для мобильного 
...
---
API развёрнут на Render: https://perevalsregister.onrender.com  
Документация к API доступна по ссылке: https://perevalsregister.onrender.com/docs

Для корректной работы API в базе данных есть тестовые данные.  
`"pereval_area": 4`  
`"user": 1`  
`"actyvity_type: 1"`  
`"winter", "spring", "summer, "autumn"` - должны соответствовать последовательности `('1A', '1B', '2A', '2B', '3A', '3B')`

`email` пользователя для метода `GET /submitData/ ?user__email=email` - `panichevr@gmail.com`.
